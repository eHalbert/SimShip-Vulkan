...
void main()
{
    ...
    vec3 N = normalize(vec3(grad.x, grad.y, grad.z)); 
    ...
    // Shadow
	float shadow = (ubo.bShowShadow > 0) ? ShadowCalculation(N) : 0.0;                      

    // Fresnel
    float F0 = 0.020018673;
    float F = F0 + (1.0 - F0) * pow(1.0 - VdotN, 5.0);
    F *= distFactor;    // Less Fresnel if far away
    ...
    if (ubo.sunDir.y > -sunSolidAngle) 
    {
        ...
        float sunDisk = pow(max(dot(R, sunDirStable), 0.0), 256.0);
    ...
    // Submarine refraction + extinction (Beer-Lambert) I = I0 × exp(-coeff_extinction × distance)
    const vec3 waterExtinction = vec3(0.5, 0.3, 0.2);
    ...
    // Alpha
    FragColor.a = mix(1.0, 1.0 - ubo.transparency, VdotN);
