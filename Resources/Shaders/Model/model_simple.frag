...
void main() 
{
    ...
    vec3 V = normalize(view.position - fragPos);    // Direction vers la caméra
    vec3 L = normalize(light.position - fragPos);   // Direction vers la lumière
    vec3 H = normalize(V + L);                      // Vecteur médian
    ...
    vec3  compSample = hasCompTex ? texture(meshCompTexture, outTexCoord).rgb : vec3(0.0);

    float ao        = hasCompTex ? compSample.r : 1.0;                  // R : Ambient Occlusion
    ...
    vec3  specular    = light.specular * (nominator / max(denominator, 0.001)) * light.specularIntensity * dot(N, L);
